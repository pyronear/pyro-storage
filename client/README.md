...
```python
source_token = requests.post(urljoin(API_URL, f"sources/{cam_id}/token"), headers=admin_headers, timeout=5).json()[
        "access_token"
    ]

camera_client = Client(source_token, "http://localhost:5050", timeout=10)
