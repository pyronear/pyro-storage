...
ROUTES: Dict[str, str] = {
    ...
    # SOURCES
    #################
    "sources-heartbeat": "/sources/heartbeat",
...
class Client:
    ...
    def fetch_sources(self) -> Response:
        ...
        >>> response = api_client.fetch_sources()

        Returns:
            HTTP response
        """
        return requests.get(
            self.routes["sources-fetch"],
    ...
    def heartbeat(self) -> Response:
        ...
        """
        return requests.patch(self.routes["sources-heartbeat"], headers=self.headers, timeout=self.timeout)
