...
class S3Service:
    ...
    @staticmethod
    def resolve_bucket_name(_id: int) -> str:
        return f"{settings.SERVER_NAME}-alert-api-{_id!s}"


async def upload_file(file: UploadFile, _id: int, camera_id: int) -> str:
    ...
    await file.seek(0)
    bucket_name = s3_service.resolve_bucket_name(_id)
