...
def main(args):
    ...
    # create a user
    payload = {"source_id": 1, "login": user_login, "password": user_pwd, "role": "agent"}
    ...
    cam_id = api_request("post", f"{args.endpoint}/sources/", agent_auth, payload)["id"]

    cam_token = requests.post(
        f"{args.endpoint}/sources/{cam_id}/token",
        timeout=5,
    ...
    api_request("delete", f"{args.endpoint}/sources/{cam_id}/", superuser_auth)
